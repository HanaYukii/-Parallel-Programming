...
int main(int argc, char **argv){
    ...
    while(iteration--){
        ...
        if(cur > 0){
            MPI_Recv(&next[(st - 1) * W], W, MPI_INT, cur - 1, 0, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
        }
        if(cur < sz - 1){
            MPI_Recv(&next[ed * W], W, MPI_INT, cur + 1, 0, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
        }
        MPI_Barrier(MPI_COMM_WORLD);
    ...
    for(int i = st ; i < ed ; i++){
        for(int j = 0 ; j < W ; j++){
            if(temp[i * W + j] < Min1) Min1 = temp[i * W + j];
        }
    }
    MPI_Reduce(&Min1,&Min2,1,MPI_INT,MPI_MIN,0,MPI_COMM_WORLD);
    MPI_Barrier(MPI_COMM_WORLD);
    if(cur == 0) {
